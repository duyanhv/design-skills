...
APP_DIR="${RN_APP_DIR:-/tmp/RNVerify}"
...
[[ -d "$APP_DIR" ]] || { echo "error: no app at $APP_DIR. See README.md." >&2; exit 1; }
[[ "$UDID" =~ ^[0-9A-F]{8}- ]] || {
...
for variant in variants/[A-Z]*.tsx; do
  ...
  ( cd "$APP_DIR/ios" && xcodebuild \
      ...
      build > "/tmp/rn-build-$name.log" 2>&1 ) || {
    echo "  BUILD FAILED — last errors:"
    grep -E "error:" "/tmp/rn-build-$name.log" | head -5 | sed 's/^/    /'
    echo "    full log: /tmp/rn-build-$name.log"
    continue
  }
