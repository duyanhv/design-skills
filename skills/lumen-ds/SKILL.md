...
Rules are Lumen Design System's own sentences. Severity is inferred from that wording, not declared by Lumen Design System; when a decision turns on it, follow the citation. Text marked `_[figure: …]_` stands for an image that carries information the text does not.
